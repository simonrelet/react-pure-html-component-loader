...
function renderJsxPropsSpreading(value) {
  return value.replace(bindings.STRICT_PATTERN, '{ ...$1 }');
}
...
function renderJsxConditionalTag(node, tagToVar, indent) {
  const test = node.attrs[controlsAttrs.CONDITIONALS_TEST]
...
function renderJsxLoopTag(node, tagToVar, indent) {
  const arrayName = node.attrs[controlsAttrs.LOOP_ARRAY]
    .replace(bindings.STRICT_PATTERN, '$1')
    .trim();
  const varName = node.attrs[controlsAttrs.LOOP_VAR_NAME]
    .replace(bindings.STRICT_PATTERN, '$1')
    .trim();
